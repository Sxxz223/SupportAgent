...
- `proactiveMessages` is optional and contains at most one useful later message with `id`, `content`, `category`, `priority`, `delaySeconds`, and `expiresInSeconds`. Choose `delaySeconds` from 1-30 based on whether the customer is waiting and whether a timely follow-up is useful; do not use a fixed cadence. A `followup` may ask one short question after the formal response. If it needs a customer answer, include the matching `interaction` in that proactive message so the action panel updates at the same time; never hide a required answer only in the message bubble.
