...
- `proactiveMessages` is optional and contains at most one useful follow-up with `id`, `content`, `category`, `priority`, `delaySeconds`, and `expiresInSeconds`. Choose `delaySeconds` from 1-30 based on whether the customer is waiting and whether a timely explanation is useful; do not use a fixed cadence. It must not create a new question, task, or required action.
